...
| Thời gian gán `clip_02` | Không có nhật ký thời gian để xác nhận |
| Thời gian gán `clip_01` | Không có nhật ký thời gian để xác nhận |
| Số track đã vẽ trong `clip_01` | 8 |
| Số keyframe trung bình mỗi track | Chưa xác định từ artifact hiện có; MOT không giữ cờ keyframe |
...
Ba tình huống cụ thể được phát hiện **khi đánh giá sau gán nhãn**, cần phân biệt với hồi tưởng cá nhân trong lúc thao tác:

1. **Thời điểm bắt đầu track:** nhãn ID 6 có bbox tại MOT frame 82–100 trước thời điểm track tương ứng trong reference xuất hiện. Điểm cần kiểm tra là lúc xe đủ rõ để bắt đầu gán; không thể kết luận chỉ từ số track.
2. **Thời điểm kết thúc track:** nhãn ID 4 còn bbox tại MOT frame 149–151 sau khi track reference kết thúc. Đây là đoạn cần xem lại nút Outside và ranh giới ra khỏi khung hình.
3. **Độ khít ở một số frame:** IoU của cặp ghép với gold ID 6 tại frame 108–109 khoảng 0,51; một cặp ghép với gold ID 1 tại frame 190 khoảng 0,50. Cần nhìn lại bbox và keyframe quanh những đoạn này.

Chưa có lời mô tả của tác giả về ba tình huống khó nhất trong lúc gán và quyết định khi đó. Các finding trên là kết quả chẩn đoán, không được trình bày thành những lần sửa đã thực hiện.
...
### Đối chiếu theo ba lượt QC

| Lượt | Bằng chứng hiện có | Giới hạn |
| --- | --- | --- |
| 1 — ID và timeline | Chấm `clip_01` cho IDSW = 0; không có track gold bị bỏ hoàn toàn hoặc bị phân mảnh trong chẩn đoán nhãn tay | Kết quả metric không thay cho nhật ký đã tua và kiểm bằng mắt |
| 2 — Frame đầu/cuối | Công cụ nêu đoạn ID 6 bắt đầu sớm và ID 4 kết thúc muộn | Chưa có closure do tác giả xác nhận |
| 3 — Geometry/interpolation | Công cụ liệt kê 19 cặp bbox có độ khít thấp cần xem lại; MOTP tổng thể = 0,8758 | Chưa có bằng chứng sửa keyframe sau chấm |

**Kiểm chéo:** chưa có tên reviewer, bản nhãn độc lập của reviewer hoặc biên bản review trong artifact được kiểm tra. Không ghi số lỗi tìm cho nhau khi chưa có bằng chứng. Các nhận xét chẩn đoán trong báo cáo là hỗ trợ QC sau chấm, không thay thế peer review của học viên.

Các luật cần được làm rõ khi review là: ngưỡng bắt đầu track cho xe nhỏ/mờ; thời điểm kết thúc track khi xe chạm rìa; bbox chỉ bao phần nhìn thấy khi có che khuất. Chưa có dữ liệu để khẳng định đây là các bất đồng đã xảy ra giữa hai reviewer.
...
**Qua cổng: có.** IDF1 ≥ 0,80; MOTA ≥ 0,75; MOTP ≥ 0,70. Hai dòng giống nhau vì chưa có thay đổi annotation sau khóa, không phải bằng chứng rằng một vòng rework đã được thực hiện. Nguồn: [eval_pre_gold.json](../outputs/eval_pre_gold.json) và [eval_vs_gold.json](../outputs/eval_vs_gold.json).
...
**Ranh giới cuối track — MOT frame 150–151 (CVAT 149–150), nhãn tay ID 4 / treatment ID 9 / gold ID 4.** Reference kết thúc track 4 ở frame 148. Treatment ID 9 còn ở frame 149 nhưng không còn ở frame 150–151; nhãn tay vẫn có bbox tại hai frame này, kích thước lần lượt khoảng `47,47 × 145,32` và `17,69 × 130,67` pixel, sát mép trái. Ở frame 150–151, quyết định không xuất track này của treatment phù hợp reference hơn nhãn tay, nên đây là một ca model làm tác giả cần xem lại endpoint/Outside.

Tuy nhiên ảnh frame 151 vẫn còn một dải xe rất hẹp ở mép trái. Vì vậy bằng chứng chưa cho phép nói xe đã hoàn toàn biến mất; cần đối chiếu quy định về phần xe còn nhìn thấy và ngưỡng kết thúc track với Lab Coach. Không sửa annotation chỉ để bắt chước model hoặc tăng điểm. Treatment cũng kéo track đến frame 149, muộn hơn reference một frame, nên không phải đáp án đúng tuyệt đối cho toàn đoạn.
...
Từ các finding hiện có, các cải tiến quy trình được đề xuất là:
...
- **Giữ bằng chứng trước/sau:** khóa pre-gold trước reference/model, lưu các lần đánh giá, mô tả rõ có hay chưa rework; không ghi những lần sửa chưa diễn ra.
...
Đây là đề xuất dựa trên kết quả kỹ thuật, chưa thay thế phần phản ánh cá nhân của tác giả về điều đã học và thay đổi mình muốn áp dụng.

## 7. Tệp đã nộp

Đây là kiểm kê file tại thời điểm cập nhật báo cáo, chưa xác nhận commit/push hoặc nộp VLearn. “Có trên Colab” khác với “đã có trong repo local”.

| Artifact | Repo local | Notebook Colab mới |
| --- | --- | --- |
| `annotations/clip_01/gt.txt`, `annotations/clip_02/gt.txt` | Có | Đã đọc nhãn clip chính: 609 bbox, 8 track |
| `evidence/pre-gold/clip_01/gt.txt` và `manifest.json` | Có | Không dùng lần chạy model để thay snapshot |
| `outputs/eval_pre_gold.json` | Có | Bản đánh giá snapshot lưu local |
| `outputs/eval_vs_gold.json` | Có | Đã chạy lại; bảng nhãn tay khớp |
| `reports/REPORT.md` | Đã cập nhật | Tổng hợp từ notebook mới |
| `outputs/model_bytetrack_clip_01.txt` | Chưa có | Đã tạo: 607 bbox, 16 track |
| `outputs/model_reid_clip_01.txt` | Chưa có | Đã tạo: 638 bbox, 16 track; đã đọc MOT |
| `outputs/model_run_config.json` | Chưa có | Đã tạo và in cấu hình |
| `outputs/eval_bytetrack_vs_gold.json` | Chưa có | Đã tạo và in metric/chẩn đoán |
| `outputs/eval_reid_vs_gold.json` | Chưa có | Đã tạo và in metric/chẩn đoán |
| `outputs/eval_reid_vs_me.json` | Chưa có | Đã tạo và in metric/chẩn đoán |
| `GUIDELINE_MINI.md` với các ca thực tế | Chưa xác nhận hoàn tất | Đề xuất bổ sung ở mục 6 |
| `reports/review_partner.md` | Chưa có biên bản reviewer/closure thực tế | Không thay peer review bằng chấm model |

Trước khi commit bản nộp tiếp theo, cần tải sáu file model/config/evaluation còn thiếu từ `/content/Day3-Lab/outputs/` vào `outputs/` của repo, rồi commit chúng cùng báo cáo và các JSON đánh giá cần nộp. Các file stretch `stretch_reid_0_7.yaml`, `stretch_reid_0_8.yaml`, `stretch_reid_0_9.yaml` và `tmp_reid_0_7.txt`, `tmp_reid_0_8.txt`, `tmp_reid_0_9.txt` hiện cũng có trên Colab; lưu chúng nếu cần tái lập phần khảo sát. Không commit gold, weights hoặc ZIP.

**Nguồn đối chiếu:**

- [Pre-gold manifest](../evidence/pre-gold/clip_01/manifest.json) và [đánh giá nhãn tay](../outputs/eval_vs_gold.json).
- [Colab mới — cấu hình và hai run](https://colab.research.google.com/drive/1oMiY09kHddP2kTy3-Ecpo4Y4Nw-Y9qgY?hl=vi#scrollTo=uvcRj10ic4cX).
- [Colab mới — chẩn đoán ByteTrack và ReID vs gold](https://colab.research.google.com/drive/1oMiY09kHddP2kTy3-Ecpo4Y4Nw-Y9qgY?hl=vi#scrollTo=a1FiXZaIc4cY).
- [Colab mới — bảng tổng hợp](https://colab.research.google.com/drive/1oMiY09kHddP2kTy3-Ecpo4Y4Nw-Y9qgY?hl=vi#scrollTo=-tY4zO5uc4cZ).
- [Colab mới — frame bất đồng](https://colab.research.google.com/drive/1oMiY09kHddP2kTy3-Ecpo4Y4Nw-Y9qgY?hl=vi#scrollTo=qGE7THI5c4ca).
- [Colab mới — khảo sát appearance](https://colab.research.google.com/drive/1oMiY09kHddP2kTy3-Ecpo4Y4Nw-Y9qgY?hl=vi#scrollTo=a-OUWMKOc4cb).
- [Ảnh frame 151](../data/clips/clip_01/img1/000151.jpg), [ảnh frame 190](../data/clips/clip_01/img1/000190.jpg), MOT treatment đọc qua trình xem file Colab, nhãn local và reference do Lab Coach cung cấp.
- [Quy định artifact](../SUBMISSION.md) và [mẫu báo cáo](REPORT_TEMPLATE.md).

**Phạm vi hỗ trợ:** AI hỗ trợ đọc output, đối chiếu frame/ID và biên soạn báo cáo; không thay đổi code notebook, nhãn CVAT hoặc snapshot pre-gold trong bước cập nhật. Thời lượng gán nhãn, trải nghiệm cá nhân và biên bản peer review chưa có bằng chứng vẫn được nêu rõ.
